...
// type types.Uint256 [UINT256_SIZE]byte

var EMPTY_HASH = types.Uint256{}
...
type CompactMerkleTree struct {
	...
	hasher    TreeHasher
	hashStore HashStore
	rootHash  types.Uint256
	treeSize  uint32
}

// NewTree returns a CompactMerkleTree instance
func NewTree(tree_size uint32, hashes []types.Uint256, store HashStore) *CompactMerkleTree {
...
func (self *CompactMerkleTree) Hashes() []types.Uint256 {
	return self.hashes
}
...
func (self *CompactMerkleTree) Marshal() ([]byte, error) {
	length := 4 + len(self.hashes)*types.UINT256_SIZE
...
func (self *CompactMerkleTree) UnMarshal(buf []byte) error {
	...
	if len(buf) < 4+int(nhashes)*types.UINT256_SIZE {
		return errors.New("Too short input buf length")
	}
	hashes := make([]types.Uint256, nhashes, nhashes)
	for i := 0; i < int(nhashes); i++ {
		copy(hashes[i][:], buf[4+i*types.UINT256_SIZE:])
	}
...
func (self *CompactMerkleTree) _update(tree_size uint32, hashes []types.Uint256) {
...
func (self *CompactMerkleTree) Root() types.Uint256 {
...
func (self *CompactMerkleTree) GetRootWithNewLeaf(newLeaf types.Uint256) types.Uint256 {
...
func (self *CompactMerkleTree) cloneMem() CompactMerkleTree {
	...
	temp.hashes = make([]types.Uint256, len(self.hashes))
...
func (self *CompactMerkleTree) GetRootWithNewLeaves(newLeaf []types.Uint256) types.Uint256 {
	tree := self.cloneMem()
...
func (self *CompactMerkleTree) Append(leafv []byte) []types.Uint256 {
...
func (self *CompactMerkleTree) appendHash(leaf types.Uint256) []types.Uint256 {
	size := len(self.hashes)
	auditPath := make([]types.Uint256, size, size)
	storehashes := make([]types.Uint256, 0)
	// reverse
...
func (self *CompactMerkleTree) DumpStatus() {
	log.NewNopLogger().Info("\n tree root:", self.rootHash)
	log.NewNopLogger().Info("\n tree size:", self.treeSize)
	log.NewNopLogger().Info("\n hashes size:", len(self.hashes))
	log.NewNopLogger().Info("\n hashes", self.hashes)
	log.NewNopLogger().Info("\n mintree_h", self.mintree_h)
}
...
func (self *CompactMerkleTree) merkleRoot(n uint32) types.Uint256 {
...
func (self *CompactMerkleTree) ConsistencyProof(m, n uint32) []types.Uint256 {
...
// m, n 1-based
func (self *CompactMerkleTree) subproof(m, n uint32, b bool) []types.Uint256 {
	offset := uint32(0)
	...
	for m < n {
		...
		if m <= k {
			...
			subhashes := make([]types.Uint256, len(pos), len(pos))
			for p := range pos {
				pos[p] += offset + k*2 - 1
	...
	length := len(hashes)
	reverse := make([]types.Uint256, length, length)
...
func (self *CompactMerkleTree) MerkleInclusionLeafPath(data []byte, m, n uint32) ([]byte, error) {
	...
	hashes := make([]types.Uint256, 0, depth)
	...
	length := len(hashes)
	sink := types.NewZeroCopySink(nil)
...
// InclusionProof returns the proof d[m] in D[0:n]
// m zero based index, n size 1-based
func (self *CompactMerkleTree) InclusionProof(m, n uint32) ([]types.Uint256, error) {
	if m >= n {
		return nil, errors.New("wrong parameters")
	} else if self.treeSize < n {
		return nil, errors.New("not available yet")
...
*/
func (self *MerkleVerifier) VerifyLeafHashInclusion(leaf_hash types.Uint256,
	leaf_index uint32, proof []types.Uint256, root_hash types.Uint256, tree_size uint32) error {
...
func (self *MerkleVerifier) calculate_root_hash_from_audit_path(leaf_hash types.Uint256,
	node_index uint32, audit_path []types.Uint256, tree_size uint32) (types.Uint256, error) {
...
func (self *MerkleVerifier) VerifyConsistency(old_tree_size,
	new_tree_size uint32, old_root, new_root types.Uint256, proof []types.Uint256) error {
	...
	*/
	...
	lenp := len(proof)
	pos := 0
	var new_hash, old_hash types.Uint256
