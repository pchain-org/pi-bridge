...
func TestMerkleLeaf3(t *testing.T) {
	hasher := TreeHasher{}
	leafs := []types.Uint256{hasher.hash_leaf([]byte{1}),
...
func TestCompactMerkleTree_GetRootWithNewLeaves(t *testing.T) {
	...
	leaves := make([]types.Uint256, N)
	for i := 0; i < N; i++ {
		leaves[i][:][0] = byte(i)
		hash := leaves[i]
		assert.Equal(t, tree1.GetRootWithNewLeaf(hash), tree2.GetRootWithNewLeaves([]types.Uint256{hash}))
