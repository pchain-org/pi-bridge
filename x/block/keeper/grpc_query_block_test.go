...
func TestBlockQuerySingle(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{
			desc:     "First",
			request:  &types.QueryGetBlockRequest{Index: msgs[0].Index},
			response: &types.QueryGetBlockResponse{Block: &msgs[0]},
		},
		{
			desc:     "Second",
			request:  &types.QueryGetBlockRequest{Index: msgs[1].Index},
			response: &types.QueryGetBlockResponse{Block: &msgs[1]},
		},
		{
			desc:    "KeyNotFound",
			request: &types.QueryGetBlockRequest{Index: "missing"},
