...
The following is a short summary of the upgrade steps:
    1. Stopping the running Pibridge v2.0.x instance 
    1. Backing up configs, data, and keys used for running Cosmoshub-3
    1. Resetting state to clear the local Cosmoshub-3 state
    1. Copying the cosmoshub-4 genesis file to the Pibridge config folder (either after migrating an existing cosmoshub-3 genesis export, or downloading the cosmoshub-4 genesis from the mainnet github)
    1. Installing the Pibridge v4.0.x release
    1. Starting the Pibridge v4.0.x instance to resume the Cosmos hub chain at a height of <cosmoshub3 height> + 1.
...
1. Happy path: Validator successfully migrates the cosmoshub-3 genesis file to a cosmoshub-4 genesis file, and the validator can successfully start Pibridge v4 with the cosmoshub-4 genesis within 1-2 hours of the scheduled upgrade.
1. Not-so-happy path: Validators have trouble migrating the cosmoshub-3 genesis to a cosmoshub-4 genesis, but can obtain the genesis file from the Cosmos mainnet github repo and can successfully start Pibridge v4 within 1-2 hours of the scheduled upgrade.  
...
https://github.com/cosmos/pibridge/issues/569#issuecomment-767910963

## Preliminary

Many changes have occurred to the Cosmos SDK and the Pibridge application since the latest
...
__[Pibridge](https://github.com/cosmos/pibridge) application v4.0.2 is
...
supported.
- **Node Configuration**: Previously blockchain data and node configuration was stored in `~/.pibridge/`, these will
now reside in `~/.pibridge/`, if you use scripts that make use of the configuration or blockchain data, make sure to update the path.
...
can be done by backing up the `.pibridge` directory.

It is critically important to back-up the `.pibridge/data/priv_validator_state.json` file after stopping your gaiad process. This file is updated every block as your validator participates in a consensus rounds. It is a critical file needed to prevent double-signing, in case the upgrade fails and the previous chain needs to be restarted.

In the event that the upgrade does not succeed, validators and operators must downgrade back to
pibridge v2.0.15 with v0.37.15 of the _Cosmos SDK_ and restore to their latest snapshot before restarting their nodes.

## Upgrade Procedure

__Note__: It is assumed you are currently operating a full-node running pibridge v2.0.15 with v0.37.15 of the _Cosmos SDK_.

The version/commit hash of Pibridge v2.0.15: `89cf7e6fc166eaabf47ad2755c443d455feda02e`
...
1. Make sure your chain halts at the right time and date:
    ...
    ```bash
    perl -i -pe 's/^halt-time =.*/halt-time = 1613628000/' ~/.pibridge/config/app.toml
    ```

 1. After the chain has halted, make a backup of your `.pibridge` directory

    ```bash
    mv ~/.pibridge ./gaiad_backup
    ```
...
1. Export existing state from `cosmoshub-3`:
   ...
   `~/.pibridge/data/priv_validator_state.json` - or now residing in `gaiad_backup` when you made
    a backup as in the last step - and obtain it with

   ```bash
   cat ~/.pibridge/data/priv_validator_state.json | jq '.height'
   ```
...
v4.0.2 of [Pibridge](https://github.com/cosmos/pibridge). 
Cross check your genesis hash with other peers (other validators) in the chat rooms.
   ...
   **NOTE**: Go [1.15+](https://golang.org/dl/) is required!
   ...
   ```bash
   $ git clone https://github.com/cosmos/pibridge.git && cd pibridge && git checkout v4.0.2; make install
   ```
...
1. Verify you are currently running the correct version (v4.0.2) of the _Gaia_:
   ...
   ```bash
    ...
    ...
   ```
    The version/commit hash of Pibridge v4.0.2: `6d46572f3273423ad9562cf249a86ecc8206e207`
...
1. Move the new `genesis.json` to your `.pibridge/config/` directory

    ```bash
    cp genesis.json ~/.pibridge/config/
    ```
...
1. Stop your Pibridge v2.0.15 instance.
...
1. After the chain has halted, make a backup of your `.pibridge` directory
   ...
   **NOTE**: It is recommended for validators and operators to take a full data snapshot at the export
   height before proceeding in case the upgrade does not go as planned or if not enough voting power
   comes online in a sufficient and agreed upon amount of time. That means the backup of `.pibridge` should 
...
1. Install v4.0.2 of [Pibridge](https://github.com/cosmos/pibridge).
...
API server is now in-process with daemon and can be enabled/disabled by API configuration in your `.pibridge/config/app.toml`:
...
gRPC configuration in your `.pibridge/config/app.toml`
...
State Sync Configuration in your `.pibridge/config/app.toml`
