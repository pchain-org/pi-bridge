...
[Pibridge](https://github.com/cosmos/pibridge) application have been split into separate
repositories. This allows for both the Cosmos SDK and Pibridge to evolve naturally
and independently. Thus, any future [releases](https://github.com/cosmos/pibridge/releases)
of Pibridge going forward, including this one, will be built and tagged from this
repository not the Cosmos SDK.

Since the Cosmos SDK and Pibridge have now been split into separate repositories, their
...
on its current versioning path (i.e. v0.36.x ) and the [Pibridge](https://github.com/cosmos/pibridge)
application will become v2.0.x.

__[Pibridge](https://github.com/cosmos/pibridge) application v2.0.3 is
...
can be done by backing up the `.pibridge` directories.
...
- The version/commit hash of Pibridge v2.0.3: `2f6783e298f25ff4e12cb84549777053ab88749a`
...
- You may obtain the canonical UTC timestamp of the exported block by any of the following methods:
  ...
  - Through manually querying a Pibridge REST client (e.g. `/blocks/2902000`)
...
v2.0.3 of [Pibridge](https://github.com/cosmos/pibridge).

   **NOTE**: Go [1.13+](https://golang.org/dl/) is required!

   ```bash
   $ git clone https://github.com/cosmos/pibridge.git && cd pibridge && git checkout v2.0.3; make install
   ```
...
10. Move the new `genesis.json` to your `.pibridge/config/` directory
11. Replace the `db_backend` on `.pibridge/config/config.toml` to:
...
12. Note, if you have any application configuration in `gaiad.toml`, that file has now been renamed to `app.toml`:
    ...
    ```bash
    $ mv .pibridge/config/gaiad.toml .pibridge/config/app.toml
    ```
...
4. We highly recommend standing up a [testnet](https://github.com/cosmos/pibridge/blob/master/docs/deploy-testnet.md)
   with the `pibridge-2.0` release or joining the pibridge-13006 testnet. More info for joining the testnet can be
