...
Installing the `Cosmos` application on your ledger device is required before you can use either [Lunie](#lunie-io-+-ledger-nano) or [`gaiad`](#pibridge-cli-+-ledger-nano). To do so, you need to:
...
- [Install Pibridge](https://cosmos.network/docs/cosmos-hub/installation.html)
...
Use the `keyName` you set for your Ledger key and pibridge will connect with the Cosmos Ledger app to then sign your transaction.
...
- `chain-id`: The chain to which you are broadcasting the tx, such as the `pibridge-13003` testnet or `cosmoshub-2`: mainnet.
