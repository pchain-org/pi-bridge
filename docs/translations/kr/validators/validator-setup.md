...
__참고__: 이 명령어는 제네시스에서의 처리를 위해 `gentx`를 `~/.pibridge/config/gentx`에 저장합니다.
...
curl https://raw.githubusercontent.com/cosmos/testnets/master/latest/genesis.json > $HOME/.pibridge/config/genesis.json
```
...
모든 제네시스 트랜잭션을 받으시고 `~/.pibridge/config/gentx`에 저장하셨다면 다음 명령어를 실행하십시오:
...
코스모스 테스트넷의 경우 코스모스 [익스플로러](https://explorecosmos.network/validators)를 통해서 밸리데이터가 운영되고 있는지 확인하실 수 있습니다. `~/.pibridge/config/priv_validator.json` 파일의 `bech32` 인코딩이된 `address` 항목을 참고하세요.
...
밸리데이터가 자동 언본딩 되었습니다. `pibridge-8000`의 경우, 100개 블록 중 50개의 블록에 투표하지 않을 경우 언본딩 됩니다. 블록은 대략 ~2초 마다 생성되기 때문에 ~100초 이상 비활성화 상태를 유지하는 밸리데이터는 언본딩 될 수 있습니다. 가장 흔한 이유는 `gaiad` 프로세스가 멈춘 경우입니다.
...
[Unit]
Description=Cosmos Pibridge Node
