...
如果你需要使用或部署 pibridge 作为容器，你可以跳过`build`步骤并使用官方镜像，\$TAG 标识你感兴趣的版本：
...
启动将会把`gaiad`相关的所有数据放在`~/.pibridge`目录。你可以检查所创建的 genesis 文件——`~/.pibridge/config/genesis.json`。同时`gaiad`也已经配置完成并且有了一个拥有 token 的账户(stake 和自定义的代币)。
...
日志被保存在每个`./build/nodeN/gaiad/pibridge.log`文件中。你也可以直接通过 Docker 来查看日志：
