...
```bash
git clone -b v4.2.1 https://github.com/cosmos/pibridge
cd pibridge
make install
gaiad init chooseanicehandle
wget https://github.com/cosmos/mainnet/raw/master/genesis.cosmoshub-4.json.gz
gzip -d genesis.cosmoshub-4.json.gz
mv genesis.cosmoshub-4.json ~/.pibridge/config/genesis.json
```
...
To save those seeds to your settings, put the comma-separated list format seeds in `~/.pibridge/config/config.toml` in the p2p section under seeds.

## Manual Setup of a new Node

These instructions are for setting up a brand new full node from scratch.

Make sure to have the [latest pibridge version installed](./join-mainnet.md).
...
By default, the `init` command creates your `~/.pibridge` directory with subfolders `config` and `data`.
In the `config` directory, the most important files for configuration are `app.toml` and `config.toml`.

You can edit the `moniker` in the `~/.pibridge/config/config.toml` file:
...
For optimized node performance, edit the `~/.pibridge/config/app.toml` file to enable the anti-spam mechanism and reject incoming transactions with less than the minimum gas prices:
...
wget https://github.com/cosmos/mainnet/raw/master/genesis.cosmoshub-4.json.gz
gzip -d genesis.cosmoshub-4.json.gz
mv genesis.cosmoshub-4.json $HOME/.pibridge/config
```
...
Your node needs to know how to find peers. You'll need to add healthy seed nodes to `$HOME/.pibridge/config/config.toml`. The [`launch`](https://github.com/cosmos/launch) repo contains links to some seed nodes.
...
Your full-node keeps unconfirmed transactions in its mempool. In order to protect it from spam, it is better to set a `minimum-gas-prices` that the transaction must meet in order to be accepted in your node's mempool. This parameter can be set in the following file `~/.pibridge/config/app.toml`.
...
To set pruning, adjust the `pruning` parameter in the `~/.pibridge/config/app.toml` file.
...
By default, the REST API is disabled. To enable the REST API, edit the `~/.pibridge/config/app.toml` file, and set `enable` to `true` in the `[api]` section. 
...
By default, gRPC is enabled on port `9090`. In the `~/.pibridge/config/app.toml` file, you can make changes in the gRPC section. To disable the gRPC endpoint, set `enable` to `false`. To change the port, use the `address` parameter.
...
Pibridge can dump the entire application state into a JSON file. This application state dump is useful for manual analysis and can also be used as the genesis file of a new network.
