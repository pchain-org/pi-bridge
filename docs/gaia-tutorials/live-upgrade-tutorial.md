...
1. Start the network and trigger upgrade
   ...
   ```bash
   # start a pibridge application full-node
   $ gaiad start
...
2. Performing an upgrade
   ...
   In `pibridge/app/app.go`, after `upgrade.Keeper` is initialized and set in the app, set the the
   ...
   ```bash
   # create a new binary of pibridge with the added upgrade handler
   $ make install
