...
# Deploy Your Own Pibridge Testnet
...
Supporting code can be found in the [networks directory](https://github.com/cosmos/pibridge/tree/main/networks) and additionally the `local` or `remote` sub-directories.

> NOTE: The `remote` network bootstrapping may be out of sync with the latest releases and is not to be relied upon.

## Available Docker images

In case you need to use or deploy pibridge as a container you could skip the `build` steps and use the official images, \$TAG stands for the version you are interested in:

- `docker run -it -v ~/.pibridge:/root/.pibridge tendermint:$TAG gaiad init`
- `docker run -it -p 26657:26657 -p 26656:26656 -v ~/.pibridge:/root/.pibridge tendermint:$TAG gaiad start`
- ...
- `docker run -it -v ~/.pibridge:/root/.pibridge tendermint:$TAG gaiad version`
...
- [Install pibridge](./installation.md)
...
This setup puts all the data for `gaiad` in `~/.pibridge`. You can examine the genesis file you created at `~/.pibridge/config/genesis.json`. With this configuration `gaiad` is also ready to use and has an account with tokens (both staking and custom).

## Multi-node, Local, Automated Testnet

From the [networks/local directory](https://github.com/cosmos/pibridge/tree/main/networks/local):
...
Logs are saved under each `./build/nodeN/gaiad/pibridge.log`. You can also watch logs
...
The following should be run from the [networks directory](https://github.com/cosmos/pibridge/tree/main/networks).
