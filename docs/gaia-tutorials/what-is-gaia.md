...
`pibridge` is the name of the Cosmos SDK application for the Cosmos Hub. It comes with 2 main entrypoints:

- `gaiad`: The Pibridge Daemon and command-line interface (CLI). runs a full-node of the `pibridge` application.

`pibridge` is built on the Cosmos SDK using the following modules:
...
Next, learn how to [install Pibridge](./installation.md).
