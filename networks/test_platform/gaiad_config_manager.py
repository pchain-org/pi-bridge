...
target_dir = working_directory + 'mytestnet/node0/pibridged/config/'
...
def get_validator_pubkey(target_dir):
    val_pub_key = subprocess.check_output(['pibridged', 'tendermint', 'show-validator', '--home', target_dir.rstrip('/config')])
...
def get_validator_id(target_dir):
    ...
    # subprocess.call(['pibridged', 'init', '--home', target_dir])
    nodeid = subprocess.check_output(['pibridged', 'tendermint', 'show-node-id', '--home', str(target_dir).rstrip("config/")])
...
if len(template_replacements['replacement_genesis']) > 0:
    ...
    if len(template_replacements['replacement_genesis_make_safe']) > 0:
        # pibridged testnet --keyring-backend test --v 4
        print("Creating testnet subdirectories")
        subprocess.call(['rm', '-rf', working_directory + 'mytestnet'])
        subprocess.call(['pibridged', 'testnet', '--keyring-backend', 'test', '--v', str(safe_index)])

    # specify the output
    for node_num in range(safe_index):
        target_file = target_dir.replace("node0", "node" + str(node_num))
        target_files.append(target_file)
        # subprocess.call("pibridged init node" + str(node_num) + " -o --home "+target_file.rstrip('/config'), shell=True)
        subprocess.call('pibridged unsafe-reset-all --home ' + target_file.rstrip('/config'), shell=True)
    ...
    # pibridged migrate cosmoshub_3_genesis_export.json --chain-id=cosmoshub-4 --initial-height [last_cosmoshub-3_block+1] > genesis.json
    print("migration genesis:" + str(common_genesis))
    cmd_string = 'pibridged migrate ' + common_genesis + ' --chain-id cosmoshub-4 --initial-height 0  --replacement-cons-keys ' + working_directory + 'templates/validator_replacement_output.json > ' + working_directory + 'templates/genesis_replaced.json'
    ...
    subprocess.call(['pibridged', 'testnet', '--keyring-backend', 'test', '--v', str(num_of_nodes_to_apply)])
...
# time.sleep(60 * 10)
# tendermint_validator_set = subprocess.check_output(['pibridged', 'query', 'tendermint-validator-set']).decode("utf-8").rstrip('\n')
...
for target_file in target_files:
    ...
    proc = subprocess.Popen(['pibridged', 'start', '--home', target_file.rstrip('/config'), '--x-crisis-skip-assert-invariants'])

    # automatically terminate program (and thus all pibridged instances) after some time
